  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Search — RB Music" },
      {
        ...
        content: "Search songs, albums, artists and playlists on RB Music.",
      },
...
function SearchPage() {
  ...
  useEffect(() => {
    ...
    setLangLoading(true);
    Promise.all(
      LANGUAGES.map((l) => searchPlaylists(l.query, 8).catch(() => [] as SPlaylist[])),
    ).then((results) => {
      if (ignore) return;
      const data: Record<string, SPlaylist[]> = {};
      LANGUAGES.forEach((l, i) => {
        data[l.name] = results[i];
      });
      setLangData(data);
  ...
  return (
    <div className="space-y-8">
      ...
      {searched ? (
        <SearchResults data={searchResults} loading={loading} query={urlQ ?? ""} />
      ) : (
...
function SearchResults({
  ...
  data: { songs: SSong[]; albums: SAlbum[]; artists: SArtistFull[]; playlists: SPlaylist[] } | null;
  loading: boolean;
  query: string;
}) {
  if (loading) {
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-[1.75rem] bg-white/[3%] p-12 text-center ring-1 ring-white/[6%]">
        ...
        <p className="mt-1 text-xs text-white/30">
          Try searching for a different artist, album or song.
        </p>
      </div>
    );
  }
