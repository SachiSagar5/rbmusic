...
async function j<T>(path: string): Promise<T> {
  const r = await fetch(`${SAAVN_API}${path}`);
...
export const searchSongs = (q: string, limit = 24) =>
  j<{ results: SSong[] }>(`/search/songs?query=${encodeURIComponent(q)}&limit=${limit}`).then((d) => d.results ?? []);

export const searchAlbums = (q: string, limit = 20) =>
  j<{ results: SAlbum[] }>(`/search/albums?query=${encodeURIComponent(q)}&limit=${limit}`).then((d) => d.results ?? []);

export const searchArtists = (q: string, limit = 20) =>
  j<{ results: SArtistFull[] }>(`/search/artists?query=${encodeURIComponent(q)}&limit=${limit}`).then((d) => d.results ?? []);

export const searchPlaylists = (q: string, limit = 20) =>
  j<{ results: SPlaylist[] }>(`/search/playlists?query=${encodeURIComponent(q)}&limit=${limit}`).then((d) => d.results ?? []);
...
export const getPlaylist = (id: string) => j<SPlaylist>(`/playlists?id=${encodeURIComponent(id)}&limit=100`);
...
export const artistNames = (s: SSong) =>
